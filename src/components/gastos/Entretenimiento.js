...
const [formEntretenimiento, setFormEntretenimiento] = useState(formDataEntretenimiento);
const [formData, setFormData] = useState(formDataEntretenimiento);

const handleRangeChange = (event) => {
    const { id, value } = event.target;

    const updatedForm = formEntretenimiento.map(item => {
        if (item.id === id) {
    ...
    setFormEntretenimiento(updatedForm);
